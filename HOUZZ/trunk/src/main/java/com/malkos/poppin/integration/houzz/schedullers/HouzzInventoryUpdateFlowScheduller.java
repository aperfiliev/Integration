...
public class HouzzInventoryUpdateFlowScheduller  implements Job{

private static Logger logger = LoggerFactory.getLogger(HouzzInventoryUpdateFlowScheduller.class);	
private static final int MAXIMUM_ATTEMPT = 20;
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {			
		...
		IInventoryUpdateFlowService inventoryUpdateService = new HouzzInventoryUpdateFlowService();
		...
		while ((!isSucceed)&&(atemptsToRetry <= GlobalPropertiesProvider.getGlobalProperties().getInventoryUpdateAutoretryAttempts())) {
			...
			try{			
				...
				if(sBuilder.length() > 0){	
						RetailerAbstract retailer = RetailerManager.get_retailer(RetailerEnum.HOUZZ);
