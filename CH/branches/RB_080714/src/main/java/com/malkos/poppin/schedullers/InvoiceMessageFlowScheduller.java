...
public class InvoiceMessageFlowScheduller {
	@Autowired
	IInvoiceMessageService invoiceFlowService;	
	@Autowired
	...
	private static Logger logger = LoggerFactory.getLogger(InvoiceMessageFlowScheduller.class);
	
	public void processTask(){		
		logger.info("Order Invoice flow task scheduller : Initiating Order Invoice flow.");
		if(Synchronizer.getIsAnotherTaskRunning()){
			...
			while(attemtsToGetResource  <= MAXIMUM_ATTEMPT){
				try {
					logger.info("************Order Invoice flow Attempt to get recources: #" + attemtsToGetResource+ " **************");
					Thread.sleep(10000);
					if(Synchronizer.getIsAnotherTaskRunning() == false){
						logger.info("***********Order Invoice flow  : finally got the recources. Starting to run the task.************");
						break;
					}
			...
			if(Synchronizer.getIsAnotherTaskRunning()){	
				logger.warn("***********Order Invoice flow : Has waited to much time for another task to release the resources: skipping this run. ************");
				return;	
			}
		}
		...
		try{
			...
			invoiceFlowService.processOrderInvoicesFromPoppin();
