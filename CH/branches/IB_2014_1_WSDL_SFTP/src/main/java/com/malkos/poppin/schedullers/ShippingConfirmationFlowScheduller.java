...
public class ShippingConfirmationFlowScheduller {
	@Autowired
	IShippingConfirmationFlowService shippingFlowService;	
	@Autowired
	...
	private static Logger logger = LoggerFactory.getLogger(ShippingConfirmationFlowScheduller.class);
	
	public void processTask(){		
		logger.info("Shipping Confirmation flow task scheduller : Initiating Shipping Confirmation flow.");
		if(Synchronizer.getIsAnotherTaskRunning()){
			...
			while(attemtsToGetResource  <= MAXIMUM_ATTEMPT){
				try {
					logger.info("************Shipping Confirmation flow Attempt to get recources: #" + attemtsToGetResource+ " **************");
					Thread.sleep(10000);
					if(Synchronizer.getIsAnotherTaskRunning() == false){
						logger.info("***********Shipping Confirmation flow  : finally got the recources. Starting to run the task.************");
						break;
					}
			...
			if(Synchronizer.getIsAnotherTaskRunning()){	
				logger.warn("***********Shipping Confirmation flow : Has waited to much time for another task to release the resources: skipping this run. ************");
				return;	
			}
		}
		...
		try{
			...
			shippingFlowService.processShippingConfirmations();
			...
			if(ErrorsCollector.hasCommonErrors()){
				int count = 1;
				sBuilder.append("The following common errors has been appeared during processing the Shipping Confirmation flow : \r\n");
			...
			String errorMessage = "An unexpected error has occurred while executing the Shipping Confirmation flow. Error message : \r\n" + e.getMessage();
