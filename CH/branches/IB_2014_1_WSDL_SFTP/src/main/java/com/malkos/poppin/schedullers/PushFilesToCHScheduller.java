...
public class PushFilesToCHScheduller {
	@Autowired
	IFilesStorageService fileStorageService;	
	@Autowired
	...
	private static Logger logger = LoggerFactory.getLogger(PushFilesToCHScheduller.class);
	
	public void processTask(){		
		logger.info("CH push files flow task scheduller : Initiating purchase order flow.");
		if(Synchronizer.getIsRemoteFileManagerConnectionBusy()){
			...
			while(attemtsToGetResource  <= MAXIMUM_ATTEMPT){
				try {
					logger.info("************CH push files Attempt to get recources: #" + attemtsToGetResource+ " **************");
					Thread.sleep(10000);
					if(Synchronizer.getIsRemoteFileManagerConnectionBusy() == false){
						logger.info("***********CH push files flow  : finally got the recources. Starting to run the task.************");
						break;
					}
			...
			if(Synchronizer.getIsRemoteFileManagerConnectionBusy()){	
				logger.warn("***********CH push files flow : Has waited to much time for another task to release the resources: skipping this run. ************");
				return;	
			}
		}
		...
		try{
			...
			fileStorageService.pushFilesToStorage();
			StringBuilder sBuilder = new StringBuilder();
			List<String> attachments = new ArrayList<>();
			
			if(ErrorsCollector.hasRemoteFileManagerErrorMessages()){
				int count = 1;
				sBuilder.append("The following common errors has been appeared during processing the CH push files flow : \r\n");
			...
			String errorMessage = "An unexpected error has occurred while executing the CH push files flow. Error message : \r\n" + e.getMessage();
