...
public class PushFilesToSpsScheduller {
	@Autowired
	...
	private static Logger logger = LoggerFactory.getLogger(PushFilesToSpsScheduller.class);
	
	public void processTask(){
		if(SchedulledTasksSynchronizer.getEnvironmentIsReady()){
			logger.info("SPS push files flow task scheduller : Initiating purchase order flow.");
			if(SchedulledTasksSynchronizer.getIsRemoteFileManagerConnectionBusy()){
				...
				while(attemtsToGetResource  <= MAXIMUM_ATTEMPT){
					try {
						logger.info("************SPS push files Attempt to get recources: #" + attemtsToGetResource+ " **************");
						Thread.sleep(10000);
						if(SchedulledTasksSynchronizer.getIsRemoteFileManagerConnectionBusy() == false){
							logger.info("***********SPS push files flow  : finally got the recources. Starting to run the task.************");
							break;
						}
				...
				if(SchedulledTasksSynchronizer.getIsRemoteFileManagerConnectionBusy()){	
					logger.warn("***********SPS push files flow : Has waited to much time for another task to release the resources: skipping this run. ************");
					return;	
				}
			}
			...
			try{
				...
				if(ErrorsCollector.hasCommonErrors()){
					int count = 1;
					sBuilder.append("The following common errors has been appeared during processing the SPS push files flow : \r\n");													
