...
public class IndigoBulkImportValidator implements IValidator {
	@Override
	public void validate(PoDocument document) {
		...
		for(PoDocumentItem pdi : document.getPoDocumentItemList()){
			if(pdi.getUPC() == null)
				requiredFieldsMissedList.add("ConsumerPackageCode");
			...
			if(!document.getRetailer().getPoppinAssortment().containsKey(pdi.getUPC())){
				commonError += "Could not find appropriate to " + pdi.getUPC() + " inventory item in NetSuite.\r\n";
				wrongVendorPartNumbersList.add(pdi.getUPC());
			}				
		}
