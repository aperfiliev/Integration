...
public class SoapMessagesSaver {
	...
	public SoapSaverResponse saveSoapMessage(Document request, Document response, SoapMessageType messageType, String messageDetails) throws TransformerException{
		...
		String requestFilePath = GlobalPropertiesProvider.getGlobalProperties().getCurrentRequestRespnonseDirectory() + File.separator + messageDetails+"request_" + messageType.toString().toLowerCase() + "_" + dateFormat.format(now) + ".xml";
		String responseFilePath = GlobalPropertiesProvider.getGlobalProperties().getCurrentRequestRespnonseDirectory() + File.separator +messageDetails+ "response_" + messageType.toString().toLowerCase() + "_" + dateFormat.format(now) + ".xml";
