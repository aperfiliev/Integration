...
public class StaplesDropShipValidator implements IValidator{
	...
	public void validate(PoDocument document) {
		...
		if (document.getShipLocationNumber() == null)
			requiredFieldsMissedList.add("shipLocationNumber");
		...
		if(document.getShipLocationNumber() != null){
			...
			if(!addressMappings.containsKey(document.getShipLocationNumber()))
				commonError += "Could not find appropriate to shipLocationNumber = "+ document.getShipLocationNumber() + " shipping address in NetSuite. \r\n";
		}
